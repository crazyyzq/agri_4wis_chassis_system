                                    ...
                                    vehicle_actuator_command_t *out)
{
    ...
    for (uint32_t wheel = 0U; wheel < ECU_WHEEL_COUNT; ++wheel) {
        ...
        out->target_wheel_speed_mps[wheel] = input->target_wheel_speed_mps[wheel];
    }
}

static void build_positive_ackermann_targets(float speed_mps,
                                             float steer_deg,
                                             const motion_control_limits_t *limits,
                                             vehicle_actuator_command_t *out)
{
    ...
    four_wheel_kinematics_build_ackermann(speed_mps, steer_deg, &geometry, &kinematics);
    apply_kinematics_output(&kinematics, out);
}

static void build_reverse_ackermann_targets(float speed_mps,
                                            float steer_deg,
                                            const motion_control_limits_t *limits,
                                            vehicle_actuator_command_t *out)
{
    /* At this layer speed_mps is already expressed in the fixed vehicle frame.
    ...
    four_wheel_kinematics_output_t kinematics;
    four_wheel_kinematics_geometry_t geometry = geometry_from_limits(limits);
    four_wheel_kinematics_build_reverse_ackermann(speed_mps, steer_deg, &geometry, &kinematics);
    apply_kinematics_output(&kinematics, out);
}

static void build_spin_targets(float speed_mps,
                               vehicle_actuator_command_t *out)
{
    four_wheel_kinematics_output_t kinematics;
    four_wheel_kinematics_build_spin(speed_mps,
                                    ...
                                    vehicle_actuator_command_t *out)
{
    ...
    float limited_speed_mps = clampf(speed_mps, limits->max_speed_mps);
    float steer_deg = clampf(steer_input_deg, limits->max_steer_deg);

    /* target_speed_mps is stored in the fixed vehicle frame.  It may be
     ...
     */
    out->target_speed_mps = limited_speed_mps;

    switch (mode) {
    case ECU_MOTION_MODE_REVERSE_ACKERMANN:
        build_reverse_ackermann_targets(limited_speed_mps, steer_deg, limits, out);
        break;
    ...
    default:
        build_positive_ackermann_targets(limited_speed_mps, steer_deg, limits, out);
        break;
    }
}
