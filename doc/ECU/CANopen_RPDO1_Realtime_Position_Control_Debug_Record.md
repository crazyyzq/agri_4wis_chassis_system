...
> **归档状态（2026-07-12）**：本文记录的是 `compact6` 与 `current7` 的历史比较，不是生产操作手册。现行 Node1–13 PDO 契约固定为 `current7 + sync1`：速度 RPDO0、位置 RPDO1 都携带 `6060` 模式字节，RPDO3 使用 `6040 + 6060 + 2340`。文中任何“推荐 compact6 作为默认方案”或 `async255` 的结论均已失效，只保留为测试证据。生产配置以 `doc/CANOPEN_PDO_MAPPING_RECORD_V1.md`、`tools/canopen_pdo_config/README.md` 和代码配置为准。
...
| compact6 + sync1 | 四轴通过，0 EMCY | 有同步 TPDO 反馈 | 历史候选，当前不采用 |
