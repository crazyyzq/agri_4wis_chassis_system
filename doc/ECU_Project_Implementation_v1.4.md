...
生产固件在禁用态完成 SDO 配置，再用四轴 RPDO1 组和公共 SYNC 完成同步 switch-on/enable；从各轴实测位置捕获起点，预装 3 组静止点，运行中每 20 ms 严格发送四轴 RPDO2 后一个公共 SYNC。正常速度 6 mm/s、加速度 8 mm/s²。摇杆回中不是立即停机，而是减速并以较低速度双向找平；达到共同目标、轴差不超过 3 mm 且实际速度稳定为零后才统一失能抱闸。安全停机仍立即失能。低于 10 mm 只允许伸长，高于 490 mm 只允许缩短，回到安全区后恢复双向。Node13 泵 PDO 仅在命令变化或确认失速重试时发送，禁止周期性泵 SYNC 插入四腿点流。BC2 插补模式可能在正常可动时报告状态字 `0x162F`，因此硬故障以厂家锁存故障 `0x2183` 为准；清故障后从新实测位置恢复，不复位驱动器。
