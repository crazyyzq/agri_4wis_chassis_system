...
BC2 的 SW 拨码只设置 A 轴节点号，B 轴节点号等于 A 轴节点号加 1。例如前 BC2 拨码为 9 时，A 轴为节点 9，B 轴为节点 10；后 BC2 拨码为 11 时，A 轴为节点 11，B 轴为节点 12。虽然 B 轴物理端子命名为 OUT4/IN7/IN8，但 CANopen 访问必须通过 B 轴自己的节点号进行。B 轴节点访问 `0x2190` 时仍读轴内 IN2/IN3 对应的限位位。抱闸由驱动器内部抱闸控制器处理，ECU 只能通过 CiA-402 controlword 请求驱动器进入/退出可运动状态，不得写 `0x2194` 或驱动器 OUT 位来控制抱闸。
...
MOS8/EX_OUT8 是高压继电器物理输出，GPIO 高电平闭合继电器。收到上高压请求后 MOS8 置位并保持；只有 CH4 左拨明确失能、或 CPU0 复位后的安全默认态，才释放 MOS8。BMS 高压接触器反馈稳定后，ECU 再发送 DCDC12V 和逆变启动指令，避免 12V/DCAC 在高压未闭合时反复启动。

下电条件：CH4 左拨保持 350 ms 后立即释放 MOS8 并撤销高压请求；上层有序下电仍要求 P、停车和抱闸确认、液压停止、液压阀关闭、无地隙/行距/模式过渡、无受控停车中。
