...
> **文档状态（2026-07-12）**：本文件描述当前 CPU0 架构与实施约束。
...
现场接收机方向已确认：右摇杆物理向上对应 CH2 低值（小于 1150 ppm），请求伸腿，电机反转，位置向负方向走；物理向下对应 CH2 高值（大于 1850 ppm），请求收腿，电机正转，位置向正方向走；1150～1850 ppm 保持。实测换算为 `131072 count/rev`、`12 rev/10 mm`，正常操作范围为 10～490 mm，对应约 `-1572864 .. -77070336 count`；`-10000 count` 仅保留为接近机械零位的内部硬边界，不是遥控目标。

生产固件采用分析仪成功脚本同一条时序契约：配置使能后至少等待 1.5 s，并以 100 ms 周期确认四轴连续 500 ms 稳定；随后从各轴实测位置捕获起点，预装 3 组静止点；运行中每 20 ms 严格发送四轴 RPDO2 后一个公共 SYNC，速度 5 mm/s、加速度 8 mm/s²；连续 5 个反馈周期满足目标误差和最终轴差均不超过 3 mm 后才统一失能抱闸。Node13 泵 PDO 仅在命令变化或确认失速重试时发送，禁止周期性泵 SYNC 插入四腿点流。BC2 插补模式可能在正常可动时报告状态字 `0x162F`，因此硬故障以厂家锁存故障 `0x2183` 为准；清故障后从新实测位置恢复，不复位驱动器。
...
液压站 Node13 只允许反转，当前工作点为 1500 rpm，最高允许 2400 rpm；无有效液压阀请求时命令为 0 rpm。液压动作采用泵先阀后的反馈联锁：高压反馈和互锁后的阀请求有效时先启动泵，只有新鲜 TPDO 连续确认实际反转速度超过 800 rpm 才打开阀。800 rpm 仅用于首次建压确认，压力就绪后不因短时速度波动反复开关阀；反馈过期、故障、正转、CAN3 离线或请求取消时立即关阀，阀向切换至少全关 10 ms。若阀 1/2、3/4、5/6 任一对被同时请求，软件清掉该对阀，液压站也不得因为这次互锁后的空输出继续打压。
