...
阀 1 回收液压杆并抬升前悬挂，阀 2 伸长液压杆并降低前悬挂。后悬挂阀组描述仍有冲突，默认配置为 `0`，选择后悬挂不会启动液压站。
...
CH2 变地隙、CH14 变轮距和 CH5 前后悬挂方向是相互独立的请求，可以同时存在。R1/R2 只锁存前/后悬挂选择，不改变 CH2 或 CH14。最终液压输出仍必须经过阀组映射、互斥阀检查、高压反馈和安全层；未确认的阀组配置为 `0` 时，相应请求不得启动液压站。
...
6. 液压阀 5/6 的后悬挂语义仍需确认；当前继电器盒 1.5 已确认 MOS1/3/5/7/9/11 分别接阀 1/2/3/4/5/6，阀 1/2 为前悬挂、阀 3/4 为变轮距，且 1/2、3/4、5/6 软件互锁；
