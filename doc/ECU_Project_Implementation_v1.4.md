...
现场方向和比例定义：行走电机正转对应车辆正方向行走；转向电机目标为负时向右转，目标为正时向左转；转向目标 500000 counts 对应 45°，即 1° 对应约 11111.111 counts。
...
**CH13 不参与普通事件抑制。** 首次有效 CH13 样本只建立急停基线；之后无论域切换、基线同步还是普通防抖保护期间，只要 CH13 稳定位置相对基线发生变化，必须立即触发远程急停。
...
急停来源：CH13 稳定位置发生变化、SBUS 超时、SBUS failsafe、连续解码异常、遥控可信度异常。

CH13 的急停判定优先于普通离散按键去抖和域切换基线同步。首次有效 CH13 样本只作为基线；之后只要 CH13 的稳定位置发生变化，无论低到中、中到高、高到中或高到低，均立即进入远程急停锁存。
...
下电条件：CH4 左拨保持 350 ms 后立即释放 MOS6 并撤销逻辑高压请求；上层有序下电仍要求 P、停车和抱闸确认、液压停止、液压阀关闭、无地隙/行距/模式过渡、无受控停车中。
...
- R1/R2 选择不影响 CH2 变地隙和 CH14 变轮距，三类请求可以同时存在，最终由阀组互锁和安全层决定是否输出。
...
遥控阈值：CH2 > 1850 ppm 时伸腿，电机反转，位置向负方向走；CH2 < 1150 ppm 时收腿，电机正转，位置向正方向走；1150～1850 ppm 保持。实测换算为 `131072 count/rev`、`12 rev/10 mm`，正常操作范围为 10～490 mm，对应约 `-1572864 .. -77070336 count`；`-10000 count` 仅保留为接近机械零位的内部硬边界，不是遥控目标。插补周期为 20 ms，必须使用当前反馈位置作为插补起点，禁止在无新鲜位置反馈时盲目下发运动点。
