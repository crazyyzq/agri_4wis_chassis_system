...
> **文档状态（2026-07-15）**：本文件保留原始需求和 CPU0 架构约束；当前参数、
> 工具和现场状态从 `doc/README.md` 进入，不应从本文复制已经变化的调试数字。
> CANopen PDO 字节契约以 `doc/CANOPEN_PDO_MAPPING_RECORD_V1.md` 为唯一
> 映射来源；遥控操作以 `doc/ECU/遥控操作逻辑说明书.md` 为准；分析仪的
> 历史试验文件只能作为证据，不能覆盖代码配置。默认构建 profile 为 `safe`；
> `whole_vehicle_motion` 必须显式选择，并且不等于实车验收通过。
>
> **编码器位置单位冻结**：Node1–8 为 `10000 counts/rev`，Node9–12 为
> `131072 counts/rev`，Node13 为 `10000 counts/rev`。三类换算必须使用
> 独立配置符号，禁止依据“都是 BC/BC2 驱动器”复用同一个编码器常量。
...
遥控请求和安全快照通过双缓冲邮箱交接：写者先完整更新非活动槽，再用 release 原子切换序号；读者用 acquire 读取并复核序号。发布任务被高优先级任务抢占时，读者继续使用上一份完整快照，不会读到半包，也不会因奇偶 seqlock 的“写入中”状态制造伪急停。遥控请求超过 50 ms、安全快照超过 10 ms 未更新时，执行层回到明确安全命令。
...
急停解除不是排队事件：CH13 必须持续处于中位，状态机直接检查保持时间和停车安全条件。
...
IN2 是正限位，IN3 是负限位。ECU 通过 CANopen 读取驱动器输入状态对象 `0x2190`，按位读取 IN2/IN3；不得使用 PCB 本地输入替代转向限位。行走电机抱闸线接在伺服驱动器内部抱闸控制输出上，抱闸释放电平为高电平；ECU 不得通过 PCB DIO、`0x2194` 或 OUT1 直接控制抱闸。

现场方向和比例定义：行走电机正转对应车辆正方向行走；转向电机目标为负时向右转，目标为正时向左转。按 `10000 count/rev × 490:1`，输出轴 90° 为 `1225000 count`、45° 为 `612500 count`，即约 `13611.111 count/°`。
...
| 腿 2 抬升 A 轴 | 11 | BC2 后驱动器 | 抱闸：控制信号 I/O 端子 J3 的引脚 8 OUT1；正限位：引脚 4 IN2_GP；负限位：引脚 5 IN3_GP |
| 腿 3 抬升 B 轴 | 12 | BC2 后驱动器 | 抱闸：引脚 17 OUT4；正限位：引脚 13 IN7_GP；负限位：引脚 14 IN8_GP |
| 腿 4 抬升 B 轴 | 10 | BC2 前驱动器 | 抱闸：引脚 17 OUT4；正限位：引脚 13 IN7_GP；负限位：引脚 14 IN8_GP |
| 液压站电机 | 13 | BC 系列 | 由 CANopen 速度/使能命令控制 |

当前软件数组顺序固定为腿 1、腿 2、腿 3、腿 4，即 CAN3 抬升节点顺序为 `9, 11, 12, 10`。BC2 的 SW 拨码只设置 A 轴节点号，B 轴节点号等于 A 轴节点号加 1。例如前 BC2 拨码为 9 时，A 轴为节点 9，B 轴为节点 10；后 BC2 拨码为 11 时，A 轴为节点 11，B 轴为节点 12。虽然 B 轴物理端子命名为 OUT4/IN7/IN8，但 CANopen 访问必须通过 B 轴自己的节点号进行。B 轴节点访问 `0x2190` 时仍读轴内 IN2/IN3 对应的限位位。抱闸由驱动器内部抱闸控制器处理，ECU 只能通过 CiA-402 controlword 请求驱动器进入/退出可运动状态，不得写 `0x2194` 或驱动器 OUT 位来控制抱闸。
...
现场确认的 16 路 SBUS 通道定义如下，代码必须以此表为准：
...
| CH13 | 暂停按键 | 遥控急停/复位 |
| CH14 | 右肩波轮 | 变行距输入 |
| CH15 | R1 按键 | 正阿克曼/自转请求 |
| CH16 | R2 按键 | 反阿克曼/蟹行请求 |
...
3. 稳定状态变化只生成一次事件；
4. 首帧、SBUS 重连和 HOME 域变化后必须抑制旧的普通按键事件；
...
**CH13 不参与普通事件抑制。** 去抖后的低位和高位都是持续有效的急停请求；只有中位可进入解除流程。因此上电或重连时 CH13 已在端点，也不会被错误当作无害基线。
...
急停来源：CH13 稳定低位/高位、SBUS 超时、SBUS failsafe、连续解码异常、遥控可信度异常。

CH13 经过统一 80 ms 去抖后，低位或高位持续锁存急停；中位不会自动清除，必须稳定保持至少 1 s，并同时满足零速、抱闸有效、无 A 类故障。HOME、档位、电源或 CH13 若稳定停在无效阈值间隙，则按遥控可信度故障处理。

```text
CLEAR → LATCHED：任一急停来源
LATCHED → RESET_REQUESTED：CH13 长按至中位且满足安全前置
RESET_REQUESTED → CLEAR_WAIT_NORMAL：安全条件稳定满足
CLEAR_WAIT_NORMAL → CLEAR：CH13 继续保持中位
```
...
CH4 右拨保持 350 ms 为上高压请求；左拨保持 350 ms 为失能/下高压请求。CH4 高位阈值按 SBUS PPM 等效值判断，当前 `high_min=1800`，即 1800～1950 均视为高位。

上电条件：P、零速、CH3 低位、CH1/CH2/CH14 中位、遥控链路在线且已 arm、无急停、无 A 类故障、CAN1 电源网络在线。全车调试固件中，上高压前如果驱动器尚未发布 TPDO，允许在油门低位、未请求高压、无 A 类故障时使用启动前静止窗口解除急停/准入；高压请求建立后，零速判断必须回到 CAN2 TPDO 实际速度反馈，当前容差为 ±3 rpm。

继电器盒接线 1.5 中，MOS6/EX_OUT6 接电池钥匙/高压钥匙控制端，MOS 导通即请求上高压；MOS8/EX_OUT8 为预留 3，不再是高压继电器物理输出。收到上高压请求后，CPU0 通过 `ECU_DIO_HIGH_VOLTAGE_RELAY_MASK` 锁存 MOS6；继电器盒控制端低电平有效，但 ECU GPIO 侧必须输出高电平使 MOS 导通并把控制端拉低。BMS 高压接触器反馈稳定后，ECU 再发送 DCDC12V 和逆变启动指令，避免 12V/DCAC 在高压未闭合时反复启动。

下电条件：CH4 左拨保持 350 ms 后提出下电请求；只有 P 档、停车和抱闸确认、液压停止、无调节/模式过渡且不处于受控停车时，才释放 MOS6 并撤销逻辑高压。条件不满足时拒绝本次下电并保持高压，使执行器仍可完成安全停车。
...
整车悬空调试确认：自转和蟹行不得边转向边行走。进入这两个模式后，车辆层仍按同一个遥控采样生成四轮转向目标和四轮速度目标，但 CAN2 motion/device 层必须先根据四个转向轴 `0x6064` TPDO 反馈确认已进入目标窗口，再允许 Node1-4 行走速度 RPDO 使能。当前预转向窗口为 `50000 count`，超时诊断为 `12000 ms`，转向 profile speed 使用实测稳定值 `4000000`，加速度/减速度保持 `500000 count/s²` 上限。正/反阿克曼允许边行走边连续更新最新转向目标，但不得排队旧遥控目标。
...
调节域准入：HOME 中位、实际行驶档保持 P、零速、CH3 低位、自动控制退出、无急停/A 类故障、行走与转向通信正常。进入 HOME 中位后，物理 CH5 三档拨钮不再表示实际行驶档，而作为前/后液压悬挂方向输入；实际 `active_gear` 必须保持 P。

HOME 中位下 R1/R2 是前后悬挂选择事件，不是保持电平：

- 首次进入 HOME 中位默认选择前悬挂；
- 点一下 R1 选择前悬挂，再点 R1 仍是前悬挂；
- 点一下 R2 选择后悬挂，再点 R1 返回前悬挂；
- R1/R2 选择不改变 CH2 变地隙和 CH14 变轮距的方向含义。整车首次调试时应将 CH5 保持 P 档、CH14 回中，先单独验证 CH2；组合请求仍由调节状态机、阀组互锁和安全层裁决。

### CH2：变地隙

CH2 的含义是**目标车身高度变化率**，但实际输出由四个 CAN3 抬升伺服承担，不自动打开液压阀或液压站：
...
→ 实时读取四腿 TPDO 位置
→ 从当前位置生成四腿同步插补点
→ CAN3 RPDO2(60C1:01) 四帧 + SYNC
```

现场接收机方向已确认：右摇杆物理向上对应 CH2 低值（小于 1150 ppm），请求伸腿，电机反转，位置向负方向走；物理向下对应 CH2 高值（大于 1850 ppm），请求收腿，电机正转，位置向正方向走。当前机构换算为 `131072 count/rev`、`20 rev/10 mm`，即 `262144 count/mm`；正常操作范围 10～490 mm，对应约 `-2621440 .. -128450560 count`。驱动器人工设置的机械零点不由 ECU 修改。

生产固件在禁用态完成 SDO 配置，再用四轴 RPDO1 组和公共 SYNC 完成同步 switch-on/enable；从各轴实测位置捕获起点，预装 3 组静止点，运行中每 20 ms 严格发送四轴相同的 RPDO2 绝对位置点后一个公共 SYNC。正常速度 20 mm/s、加速度 8 mm/s²，目标正常领先实测位置 2.5 mm，并在 10 mm 运行跟随窗口内通过公共速度限幅恢复，不能用四轴不同目标“追平”。摇杆回中不是立即停机，而是减速并以较低速度双向找平；达到共同目标、轴差不超过 3 mm 且实际速度稳定为零后才统一失能抱闸。安全停机仍立即失能。低于 10 mm 只允许伸长，高于 490 mm 只允许缩短，回到安全区后恢复双向。Node13 泵 PDO 仅在命令变化或确认失速重试时发送，禁止周期性泵 SYNC 插入四腿点流。BC2 插补模式可能在正常可动时报告状态字 `0x162F`，因此硬故障以厂家锁存故障 `0x2183` 为准；清故障后从新实测位置恢复，不复位驱动器。若健康四轴因生产者调度延迟耗尽插补缓冲，ECU 只清 bit4 触发边、按四轴新实测位置重新预装并同步触发，不失能、不复位、不重放旧非零目标。

禁止把 CH2 的同一个速度值原封不动发给四条腿。允许四腿存在小范围不同步，控制层应通过后续插补点逐步修正；但 CAN3 通信超时、位置反馈缺失、软限位触发或关键故障必须禁止新的运动点。
...
```text
CH14 > 1850 ppm：液压杆向外伸
CH14 < 1150 ppm：液压杆回收
1150～1850 ppm：不动作
```

变轮距阀组已确认：阀 3 使液压杆缩短、轮距变窄，阀 4 使液压杆伸长、轮距变宽。该映射只允许在 `ecu_config.h` 中配置，禁止在状态机或设备层写死阀号。

### CH5 + R1/R2：前后液压悬挂

HOME 中位时，R1/R2 选择前/后液压悬挂，CH5 三档拨钮决定方向：
...
已确认阀组：阀 1/2 控制前悬挂，阀 5/6 控制后悬挂。代码中集中配置为：

```text
ECU_HYD_VALVE_FRONT_SUSPENSION_RETRACT_MASK = valve 1
ECU_HYD_VALVE_FRONT_SUSPENSION_EXTEND_MASK  = valve 2
ECU_HYD_VALVE_REAR_SUSPENSION_RETRACT_MASK  = valve 5
ECU_HYD_VALVE_REAR_SUSPENSION_EXTEND_MASK   = valve 6
```

阀 1 回收液压杆并抬升前悬挂，阀 2 伸长液压杆并降低前悬挂；阀 5 回收液压杆并抬升后悬挂，阀 6 伸长液压杆并降低后悬挂。

液压站 Node13 只允许反转：前后悬挂阀 1/2、5/6 使用 1500 rpm，变轮距阀 3/4 使用 2400 rpm；无有效液压阀请求时命令为 0 rpm。液压动作采用泵先阀后的反馈联锁：高压反馈和互锁后的阀请求有效时先启动泵，只有新鲜 TPDO 连续确认实际反转速度超过 800 rpm 才打开阀。800 rpm 仅用于首次建压确认，压力就绪后不因短时速度波动反复开关阀；反馈过期、故障、正转、CAN3 离线或请求取消时立即关阀，阀向切换至少全关 10 ms。若阀 1/2、3/4、5/6 任一对被同时请求，软件清掉该对阀，液压站也不得因为这次互锁后的空输出继续打压。
...
typedef struct {
    ...
    int16_t assist_current_10ma[4];
    float assist_wheel_speed_limit_rpm[4];
} track_adjust_config_t;
```

CH2 变地隙、CH14 变轮距和 CH5 前后悬挂方向是相互独立的请求，可以同时存在。R1/R2 只锁存前/后悬挂选择，不改变 CH2 或 CH14。最终液压输出仍必须经过阀组映射、互斥阀检查、高压反馈和安全层；若后续接线调整导致某一阀组被配置为 `0`，相应请求不得启动液压站。

### 调节模式的强制退出

以下任一条件出现时，不论 CH2/CH14 当前位置如何，都必须进入 `ADJUST_STATE_ABORTED` 或 `TRACK_EXITING`：

- HOME 中位时 CH5 仅作为悬挂方向输入，不触发退出；若调节域未能继续强制实际行驶档保持 P，则必须退出；
- HOME 离开中位；
...
CH12、CH6、CH9 只改状态请求，分别对应左转、右转和双闪。切换时先关闭旧输出，等待 100 ms，再开启新输出。双闪左右必须共用节拍源。
...
2. CH13 各档位原始值、稳定变化触发急停、长按中位解除急停的实车时序；
3. BC/BC2 抱闸、限位、电平与反馈时序；
4. 四轮零位、方向、正反阿克曼、自转、蟹行轮速符号；
5. 行距姿态、辅助扭矩方向、最大扭矩和轮速；
6. 液压阀 5/6 已按后悬挂实现：阀 5 抬升、阀 6 下降；当前继电器盒 1.5 已确认 MOS1/3/5/7/9/11 分别接阀 1/2/3/4/5/6，阀 1/2 为前悬挂、阀 3/4 为变轮距、阀 5/6 为后悬挂，且 1/2、3/4、5/6 软件互锁；
7. 转向灯、双闪、喇叭和大灯的实车效果；当前继电器盒 1.5 已确认 MOS2 大灯、MOS4 喇叭、MOS10/12 闪光继电器预留；
