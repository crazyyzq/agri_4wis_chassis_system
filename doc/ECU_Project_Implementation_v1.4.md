...
遥控阈值：CH2 > 1850 ppm 时伸腿，电机反转，位置向负方向走；CH2 < 1150 ppm 时收腿，电机正转，位置向正方向走；1150～1850 ppm 保持。测试阶段绝对位置软限位为最短 `-10000 counts`，最长 `-8000000 counts`。插补周期为 20 ms，必须使用当前反馈位置作为插补起点，禁止在无新鲜位置反馈时盲目下发运动点。
...
液压站 Node13 只允许反转，当前工作点为 1000 rpm，最高允许 2400 rpm；无有效液压请求时命令为 0 rpm。液压动作采用泵先阀后的反馈联锁：高压反馈和互锁后的阀请求有效时先启动泵，只有新鲜 TPDO 连续确认实际反转速度超过 800 rpm 才打开阀。反馈过期、故障、速度不足、CAN3 离线或请求取消时立即关阀，阀向切换至少全关 10 ms。若阀 1/2、3/4、5/6 任一对被同时请求，软件清掉该对阀，液压站也不得因为这次互锁后的空输出继续打压。
