...
CAN3 has one strict realtime FIFO shared by Node9–12 lift interpolation and Node13 pump velocity. Ground-clearance motion uses disabled-state SDO configuration, synchronous four-axis switch-on/enable groups, measured settling, three stationary preload points, then four identical absolute-position RPDO2 points plus one SYNC every 20 ms. Normal motion is 20 mm/s with an 8 mm/s² ramp, a 2.5 mm normal target lead and a feedback governor inside the 10 mm running following-error window. Operator neutral immediately cancels the old endpoint and synchronously disables all four axes; it never keeps the preceding command alive for debounce and never starts feedback-chasing leveling. A later extend/retract request starts from fresh TPDO positions using the coherent four-axis fast-enable/preload sequence. Positions below 10 mm accept only extension toward the normal band; positions above 490 mm accept only retraction. Node13 synchronous RPDO0 is command-on-change; periodic pump refresh must not inject an extra SYNC into a running lift stream. A nonzero vendor fault latch is cleared and retried through the CAN3 owner without resetting drive position references, while a true CAN transport failure uses the bounded transport-recovery path. A healthy four-axis interpolation starvation event clears only the bit-4 trigger edge, reloads three fresh measured hold points, retriggers synchronously and resumes from a common aligned position without disabling or resetting the axes.
