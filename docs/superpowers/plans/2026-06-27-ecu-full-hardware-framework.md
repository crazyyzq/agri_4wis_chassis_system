...
**Goal:** Extend the main ECU framework from logical control scaffolding to a complete hardware-binding framework with configurable guessed mappings, device adapters, protocol boundaries and executor wiring.

**Architecture:** CPU0 remains the only safety-critical command owner. Remote/control/vehicle modules make decisions in calibrated units; device and driver adapters translate final commands to CANopen, digital outputs, analog input services, hydraulic outputs, warning lights and communication health. Any guessed hardware value is centralized in `ecu/config` macros and default config structures so the user can change wiring, node IDs, PDOs, polarity and limits without editing business logic.
...
- `ecu/config/include/ecu_config.h`, `ecu/config/src/ecu_config.c`: guessed node IDs, CAN bitrates, DO polarity, ADC scaling, hydraulic valve masks, CANopen PDO defaults and safety limits.
...
- `tests/python/*`: contract tests for guessed config centralization, device boundaries, executor wiring and CPU1 restrictions.
- `docs/ecu-main-control-architecture.md`, `docs/ecu-main-control-progress.md`, `README.md`: updated reading guide and build notes.

## Guessing rules

- Every guessed node ID, CAN ID, PDO COB-ID, baud rate, ADC scale, DO polarity, relay/MOS mask, hydraulic valve bit and limit is named with `ECU_GUESS_` or is inside a clearly named config structure.
- Business modules must not contain guessed raw numbers.
- Changing a guessed value must require editing only `ecu/config`.
...
- [x] Add tests requiring `ECU_GUESS_` hardware values in config and forbidding guessed raw node IDs/CAN IDs outside config.
